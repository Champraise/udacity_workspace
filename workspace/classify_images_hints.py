...
def classify_images(images_dir, results_dic, model):
    ...
    for key in results_dic:
       ...
       for idx in range(0, len(model_label), 1):
            model_label = ""
            pet_name = model_label[idx]
            low_pet_name = pet_name.lower().strip()
            model_label += low_pet_name             
       ...
       if truth in model_label:
           results_dic[truth[idx]].extend([model_label[idx], 1])
       ...
       # the extend function 
           ...
           results_dic[truth[idx]].extend([model_label[idx], 0])
